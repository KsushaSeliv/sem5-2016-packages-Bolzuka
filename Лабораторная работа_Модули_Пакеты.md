...
Переменная sys.path содержит список "путей", в которых Python ищет модули и пакеты при импорте. Путь при этом - произвольная строка, например, директория или zip-архив (да-да, мы можем импортировать из архивов).
